...
select Customer_Name,
       STATE,
	   DATE
from FACT_TRANSACTIONS T1 LEFT JOIN DIM_CUSTOMER C1
                          ON T1.IDCustomer=C1.IDCustomer
...
FROM FACT_TRANSACTIONS T1 LEFT JOIN DIM_LOCATION L1
                          ON T1.IDLocation=L1.IDLocation
...
Model_Name,
Manufacturer_Name,
SUM(TotalPrice)/SUM(Quantity) RATE
...
--CASE
--    WHEN T2.Year IS NOT NULL
--	THEN FORMAT(CONVERT(DECIMAL(8,2),((T1.Avg_Price-T2.Avg_Price))/CONVERT(DECIMAL(8,2))*100,T2.Avg_Price) ELSE NULL 
--	END AS 'YEARLY_%_CHANGE'
--FROM
--    (SELECT t2.Customer_Name, YEAR(t1.DATE) AS YEAR, AVG(t1.TotalPrice) AS Avg_Price, AVG(t1.Quantity) AS Avg_Qty FROM FACT_TRANSACTIONS AS t1 
--	left join DIM_CUSTOMER as t2 ON t1.IDCustomer=t2.IDCustomer
...
--AVG(TotalPrice),
--AVG(Quantity),
--AVG(case when YEAR(date) = 2003 then TotalPrice else 0 END) as avgspend_2003,
--AVG(case when YEAR(date) = 2004 then TotalPrice else 0 END) as avgspend_2004,
--AVG(case when YEAR(date) = 2005 then TotalPrice else 0 END) as avgspend_2005,
--AVG(case when YEAR(date) = 2006 then TotalPrice else 0 END) as avgspend_2006,
--AVG(case when YEAR(date) = 2007 then TotalPrice else 0 END) as avgspend_2007,
--AVG(case when YEAR(date) = 2008 then TotalPrice else 0 END) as avgspend_2008,
--AVG(case when YEAR(date) = 2009 then TotalPrice else 0 END) as avgspend_2009,
--AVG(case when YEAR(date) = 2010 then TotalPrice else 0 END) as avgspend_2010,
----the percentage of change in their spend.
--((AVG(case when YEAR(date) = 2004 then TotalPrice else 0 END))-(AVG(case when YEAR(date) = 2003 then TotalPrice else 0 END) ))
--/(nullif(AVG(case when YEAR(date) = 2003 then TotalPrice else 0 END),0))*100 change_2004,
--((AVG(case when YEAR(date) = 2005 then TotalPrice else 0 END))-(AVG(case when YEAR(date) = 2004 then TotalPrice else 0 END) ))
--/(nullif(AVG(case when YEAR(date) = 2004 then TotalPrice else 0 END),0))*100 change_2005,
--((AVG(case when YEAR(date) = 2006 then TotalPrice else 0 END))-(AVG(case when YEAR(date) = 2005 then TotalPrice else 0 END) ))
--/(nullif(AVG(case when YEAR(date) = 2005 then TotalPrice else 0 END),0))*100 change_2006,
--((AVG(case when YEAR(date) = 2007 then TotalPrice else 0 END))-(AVG(case when YEAR(date) = 2006 then TotalPrice else 0 END) ))
--/(nullif(AVG(case when YEAR(date) = 2006 then TotalPrice else 0 END),0))*100 change_2006
--from DIM_CUSTOMER C1 left  JOIN  FACT_TRANSACTIONS T1  
--                        ON C1.IDCustomer=T1.IDCustomer
--group by Customer_Name,date
--order by  AVG(TotalPrice), Customer_Name desc
